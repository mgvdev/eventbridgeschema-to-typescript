...
export const transformSchema = async (registryName: string, schemaNames: string[], path: string = './generated') => {
    ...
    const describePromise = schemaNames.map(async schemaName => {
        ...
        for (const property in schema.components.schemas.Event.properties) {
            if (schema.components.schemas.Event.properties[property].enum) {
                schema.components.schemas.Event.properties[property]['tsEnumNames'] = schema.components.schemas.Event.properties[property].enum.map((e: string) => `"${e}"`)
            }
        }

        const typescript = await compile(schema.components.schemas.Event, schemaName, {enableConstEnums: true, declareExternallyReferenced: true})
